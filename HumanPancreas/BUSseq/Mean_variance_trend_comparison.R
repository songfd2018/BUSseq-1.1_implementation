...
set.seed(123)
Num_sel <- table(w_BUSseq[1:nb[b]+sum(nb[1:(b-1)])])
# Consider the second batch
cell_sel <- list()
for(k in 1:K){
  #cell_sel[[k]] <- sample(which(w_BUSseq[1:nb[2]+nb[1]] == k - 1), size = Num_sel)
  cell_sel[[k]] <- which(w_BUSseq[1:nb[b]+sum(nb[1:(b-1)])] == k - 1)
}
...
ggplot(meanvar_frame, aes(x= Mean, y= Var, colour= Category)) +
  ...
  scale_colour_manual(values = c("#F8766D","#00BFC4")) +
  xlab("Mean") + ylab("Variance") + ggtitle("Pancreas") +
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=48),
        axis.text.x = element_text(face = "bold", #color = "#993333", 
                                   size = 36), #angle = 45),
        axis.text.y = element_text(face = "bold", #color = "blue", 
