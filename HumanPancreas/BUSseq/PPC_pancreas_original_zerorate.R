...
# create the directory to store the simulated observation replications
if(!dir.exists("Replications")){
  dir.create("Replications")
}
...
# for(i in 1:100){ 
  ...
  if(iter %% num.output == 0){
    ...
    write.table(zero_rate, file = paste0("BUSseq/ZeroRate_",proj,"_v",ver,".txt"), append = TRUE, row.names = FALSE, col.names = FALSE)
    iter <- 0
  }
  iter <- iter + 1
}
